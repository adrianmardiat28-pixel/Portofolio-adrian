...
export function About() {
  return (
    <section id="about" className="cosmic-section relative overflow-visible px-6 py-28">
      {/* Cosmic divider top */}
        ...
        <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-[1fr_2fr]">
          ...
          <div className="w-full">
            ...
            <div className="reveal mx-auto max-w-3xl text-center text-lg leading-relaxed text-muted-foreground lg:mx-0 lg:text-left">
              "I am an Information Systems student specializing in Data Science with a strong
              interest in extracting value from complex data. I focus on building predictive models
            ...
            <div className="mt-16 flex flex-wrap justify-center gap-6 lg:justify-start">
              {skillGroups.map((group, i) => (
                <div
                  key={group.title}
                  className="reveal group relative w-full overflow-hidden galaxy-card rounded-2xl p-6 transition-all hover:border-primary/40 hover:shadow-glow sm:w-[320px]"
                  style={{ transitionDelay: `${i * 100}ms` }}
                >
                  <div
                    className={`absolute inset-0 -z-10 bg-linear-to-br opacity-0 transition-opacity group-hover:opacity-100 ${group.accent}`}
                  />
                  <h3 className="font-mono text-sm font-semibold text-primary">{group.title}</h3>
                  <div className="mt-4 flex flex-wrap gap-2">
                    {group.skills.map((s) => (
                      <span
                        key={s}
                        className="galaxy-chip rounded-full px-3 py-1 text-xs text-foreground/90"
                      >
                        {s}
                      </span>
                    ))}
